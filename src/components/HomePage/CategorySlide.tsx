    ...
    <div>
      ...
      >
        ...
        {categories.map((each, i) => (
          <SwiperSlide key={i}>
            <button
              onClick={() => router.push(each.name)}
              className="border w-[10rem] h-[10rem] md:w-[17rem]  md:h-[14.5rem] flex justify-center gap-3 flex-col items-center hover:bg-[#db4444] hover:text-white ease-in-out duration-300"
            >
              {each.icon}
